    ...
    public function onSave($param)
    {
        try {
            ...
            if (!empty($param['window'])) {
                TScript::create("
                    if (window.parent) {
                        window.parent.adianti_set_field_value('form_LogForm', 'dificuldade_id', '{$obj->id}');
                        window.parent.adianti_set_field_display_value('form_LogForm', 'dificuldade_id', '{$obj->nome}');
